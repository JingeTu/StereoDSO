  ...
  public:
    ...
    inline void setZero(int n, int min = 0, int max = 1, Vec10 *stats = 0, int tid = 0) {
      if (n != nframes[tid]) {
        ...
        accE[tid] = new AccumulatorXX<10, CPARS>[n * n];
        accEB[tid] = new AccumulatorX<10>[n * n];
        accD[tid] = new AccumulatorXX<10, 10>[n * n * n];
      }
    ...
    void stitchDoubleMT(IndexThreadReduce<Vec10> *red, MatXX &H, VecX &b, EnergyFunctional const *const EF, bool MT) {
      ...
      if (MT) {
        ...
        for (int i = 0; i < NUM_THREADS; i++) {
          ...
          Hs[i] = MatXX::Zero(nframes[0] * 10 + CPARS, nframes[0] * 10 + CPARS);
          bs[i] = VecX::Zero(nframes[0] * 10 + CPARS);
        }
        ...
        H = MatXX::Zero(nframes[0] * 10 + CPARS, nframes[0] * 10 + CPARS);
        b = VecX::Zero(nframes[0] * 10 + CPARS);
